...
public class StateE<T>
{
	...
	public StateE(string name)
	{
		_stateName = name;
	}

	public StateE<T> Configure(Dictionary<T, Transition<T>> transitions)
	{
	...
	public bool CheckInput(T input, out StateE<T> next)
	{
	...
	public bool Feed(T input, out StateE<T> next)
	{
		if (transitions.ContainsKey(input))
		{
			var transition = transitions[input];
